...
每版一句话；详细变更见 [docs/CHANGELOG.md](./docs/CHANGELOG.md)。

| 版本 | 日期 | 一句话 |
| --- | --- | --- |
| **0.1.2** | 2026-05-27 | 周报生成质量大改：`doc/docs` 子目录扫描 + mtime 强弱信号分类；prompt 重构让用户 `extra_prompt` 真正主导格式（custom 模板不再被技术周报覆盖）；填充词指令过滤（"好的"/"再试一次"）；可选两轮 LLM 模式（提炼 → 渲染）；等待界面加动态进度条 + 跳动点 + 循环 log 文案。 |
| **0.1.1** | 2026-05-21 | Windows 三处开箱即坏修复（控制台窗口、弹窗误关、SSH 私钥提示）+ SSH 配置完整指南。 |
| **0.1.0** | 2026-05-20 | 首个公开版本，完整端到端流程：日志收集 → 压缩聚合 → LLM 生成 → 本地存档 → 定时邮件。 |
...
- 📚 **项目文档感知** — 自动扫描项目根 + `doc/docs/` 子目录（≤ 2 层深）作为背景；按修改时间分强/弱信号
- 🎯 **格式忠实** — 模板的 `extra_prompt`（含具体示例）直接主导输出格式；内置规则不再覆盖自定义模板
- 🔁 **两轮模式**（可选） — 先提炼成果 JSON，再渲染 Markdown；显著减少过程性指令噪音，代价是 token 翻倍
